    ...
    <section aria-labelledby="experience-heading">
      <div className="flex justify-between items-center mb-4">
        <h2
          ...
          className="text-lg md:text-xl font-semibold text-zinc-900 dark:text-zinc-50"
        >
          Experience
        </h2>
        {/* <Link 
          ...
          .map((exp: Experience, index: number) => (
            <div key={index} className="group">
              <div className="flex justify-between items-baseline mb-1">
                <h3 className="font-medium text-base md:text-lg text-zinc-900 dark:text-zinc-50">
                  {exp.role}
                </h3>
              ...
              <ul className="space-y-1.5">
                {exp.highlights
                  ...
                  .map((highlight: string, idx: number) => (
                    <li
                      key={idx}
                      className="text-xs md:text-sm text-zinc-600 dark:text-zinc-400 pl-4 relative before:content-['•'] before:absolute before:left-0 before:text-zinc-400"
                    >
                      {highlight}
                    </li>
                  ))}
